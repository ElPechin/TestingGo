...
func TestAcumuladoListaNula (t *testing.T){
	...
	errEsperado := "La lista esta vacia"

	if (err.Error() != errEsperado){
		t.Error(fmt.Sprintf("Mensaje de erro esperado %s, mensaje de error recibido %s", errEsperado, err.Error()))
	}
}

func BenchmarkAcumular(b *testing.B) {
...
func BenchmarkAcumular100000(b *testing.B) {
	lista := generarLista(1000000)
	for n:= 0; n < b.N ; n++  {
		Acumular(lista)
	}
}
